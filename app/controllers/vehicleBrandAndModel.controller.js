...
exports.allVehicleBrandsAndModels = async (req, res) => {
  try {
    ...
    const row = await pool.query("SELECT * FROM VehicleBrandsAndModels");
    row.map((vehibrand) => {
      let type = VehicleTypes.find(
        (typeVehi) => typeVehi.id == vehibrand.idType
      );
...
exports.allVehicleBrandsAndModelsByIdTypeCar = (req, res) => {
  try {
    let TypeCar = VehicleTypes.find(
      (typeVehi) => typeVehi.id == req.params.idTypeVehi
    );
    let result = VehicleBrandsAndModels.filter(
      (vehibrand) => vehibrand.idType == req.params.idTypeVehi
    );
    result.map((vehi) => {
...
exports.getVehicleBrandAndModelByUseFor = (req, res) => {
  try {
    let vehicleUseFor = Vehicles.filter(
      (vehicle) => vehicle.UseForEN == req.params.UseForEn
    );
    if (vehicleUseFor.length > 0) {
      let ListVehicleDistinct = Array.from(
        new Set(vehicleUseFor.map((vehicle) => vehicle.idVehicleBrandAndModel))
      ).map((idVehicleBrandAndModel) => {
        let vehicleBrandAndType = VehicleBrandsAndModels.find(
          (brandAndType) => brandAndType.id == idVehicleBrandAndModel
        );
        let vehicle = vehicleUseFor.find(
          (typeCar) => typeCar.idVehicleBrandAndModel == idVehicleBrandAndModel
        );
...
exports.getVehicleBrandAndModelByTypeCarAndUseFor = async (req, res) => {
  try {
    const vehicleBrandsAndModelss = await pool.query(
    ...
    let ListVehiclesUseFor = await pool.query(
    ...
    if (ListVehiclesUseFor.length > 0) {
      let ListVehicleTypeCar = ListVehiclesUseFor.filter(
        (vehicle) => vehicle.idType == req.params.VehicleType
      );
