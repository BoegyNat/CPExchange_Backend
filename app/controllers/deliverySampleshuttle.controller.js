...
exports.postEditDeliverySampleShuttle = async (req, res) => {
  try {
    ...
    const result = await pool
      .query(
        "UPDATE DeliverySampleShuttle SET nameSample = ? , nameGrade = ? , lotNumber = ? , nameSender = ? , phoneSender = ? , date = ? , startTime = ? , fromPlace = ? , toPlace = ? , nameRecipient = ? , phoneRecipient = ? , detail = ? , status = ? , path = ? WHERE idDeliverySampleShuttle = ?",
        [
          NameSample,
          NameGrade,
          LotNumber,
