...
exports.userProfile = async (req, res) => {
    try {
      let result = await pool.query("SELECT * FROM Users WHERE idUser = ?", [req.params.id]);
      if (result) {
...
exports.updateLocationUser= async (req, res) => {
  try {
    ...
    const rows = await pool.query("UPDATE Users SET  lat = ? WHERE idUser = ? ", [req.body.Lat, req.body.idUser])
    const row = await pool.query("UPDATE Users SET lng = ? WHERE idUser = ? ", [req.body.Lng, req.body.idUser])
